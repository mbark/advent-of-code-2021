...
func main() {
	var first int
	var second int
	for _, line := range util.ReadInput(in, "\n") {
		split := strings.Split(line, " | ")
		signals := strings.Split(split[0], " ")
		output := strings.Split(split[1], " ")
		count, value := both(signals, output)
		first += count
		second += value
	}
...
type Mapping = map[rune]map[rune]bool

func Key(mapping Mapping) string {
...
func CopyWith(m Mapping, key rune, val rune) Mapping {
	newMap := make(Mapping, len(m))
	for di, possible := range m {
		pos := make(map[rune]bool)
		if di == key {
			pos[val] = true
		} else {
...
func PruneSingles(mapping Mapping) []rune {
	var singles []rune
	for _, possible := range mapping {
		if len(possible) == 1 {
			for p := range possible {
	...
	for _, single := range singles {
		for k, possible := range mapping {
			if len(possible) == 1 {
				continue
			}

			if _, ok := possible[single]; ok {
				deleted = true
				delete(possible, single)
				mapping[k] = possible
			}
		}
	}
	...
	return PruneSingles(mapping)
}

func both(signals []string, output []string) (int, int) {
	mapping := make(Mapping, len(digitMap))
	for _, d := range []rune{'a', 'b', 'c', 'd', 'e', 'f', 'g'} {
		mapping[d] = map[rune]bool{'a': true, 'b': true, 'c': true, 'd': true, 'e': true, 'f': true, 'g': true}
	}

	mapping = constructMapping(signals, make(map[string]bool), mapping)
	var count int
	var signalNumber string
	for _, o := range output {
		var number int
		for n, digits := range digitMap {
			if len(digits) != len(o) {
				continue
			}
		...
		signalNumber += strconv.Itoa(number)
	}

	return count, util.Str2Int(signalNumber)
}

func constructMapping(signals []string, visited map[string]bool, mapping Mapping) Mapping {
	if _, ok := visited[Key(mapping)]; ok {
		return nil
	}
	visited[Key(mapping)] = true

	nextMap := make(Mapping)
	for _, signal := range signals {
		var matches []int
		for i, digits := range digitMap {
			if len(signal) != len(digits) {
				continue
			}
		...
		for _, s := range signal {
			...
			for _, m := range matches {
				for _, b := range digitMap[m] {
					if _, ok := mapping[s][b]; ok {
						newMap[b] = true
					}
				}
			}
	...
	for d, possible := range mapping {
		var copies []Mapping
		for p := range possible {
			copies = append(copies, CopyWith(mapping, d, p))
		}

		for _, c := range copies {
			m := constructMapping(signals, visited, c)
			if m != nil {
				return m
			}
		}
...
func Print(mapping Mapping) {
	...
	for _, r := range keys {
		...
		rows = append(rows, fmt.Sprintf("%s: [%s]", string(r), strings.Join(s, ", ")))
	}
